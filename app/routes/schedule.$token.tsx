      ...
      <div className="mx-auto max-w-3xl px-4 py-8">
        ...
        {flights.length === 0 ? (
          <div className="rounded-lg border border-dashed border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 p-12 text-center">
            <p className="text-sm text-slate-500 dark:text-slate-400">No scheduled flights for this date.</p>
          </div>
        ) : (
            ...
            {flights.map((f, i) => (
              <div key={i} className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
                <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                  ...
                  <span className="text-xs font-mono text-slate-500 dark:text-slate-400">{f.routePath || `${f.originCode} \u2192 ${f.destinationCode}`}</span>
                </div>
