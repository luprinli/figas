...
import { financeDashboardTour } from "../utils/tour/definitions/finance-dashboard";
...
export async function loader({ request }: LoaderFunctionArgs) {
 ...
 ORDER BY month
    ...
    if (Number(agingRow.due_90) > 500) {
        alerts.push({
            id: "aging90",
            message: `Ã‚Â£${Number(agingRow.due_90).toLocaleString()} overdue by 90+ days`,
            severity: "amber",
...
export default function FinanceDashboard() {
    ...
    const paymentColumns: Column<Record<string, unknown>>[] = [
        { key: "booking_reference", header: "Booking", sortable: true },
        {
            ...
            render: (item) => <span className="tabular-nums font-medium">Ã‚Â£{Number(item.amount_gbp).toLocaleString()}</span>,
        },
        {
    ...
    return (
        <div className="p-6 space-y-5">
            ...
            <TourTrigger config={financeDashboardTour} />

            {/* KPI Row */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <MetricCard
                    label="Total Revenue"
                    value={`Ã‚Â£${totalRevenue.toLocaleString()}`}
                    color="emerald"
                ...
                <MetricCard
                    label="Outstanding"
                    value={`Ã‚Â£${outstanding.toLocaleString()}`}
                    ...
                    {agingBuckets.map((bucket) => (
                        <div key={bucket.label} className="text-center">
                            ...
                            <p className={`text-lg font-bold tabular-nums ${bucket.color}`}>
                                Ã‚Â£{bucket.value.toLocaleString()}
                            </p>
                        </div>
                    ))}
                </div>
