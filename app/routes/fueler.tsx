...
export const meta: MetaFunction = () => [{ title: "Fueler - FIGAS" }];
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const [pendingOrders] = await Promise.all([
    sql<Record<string, unknown>>`
      SELECT COUNT(*)::int AS count FROM fuel_orders WHERE status IN ('issued', 'fueling')
    `.execute(kdb),
  ]);

  const pendingCount = Number((pendingOrders.rows[0] as Record<string, unknown>)?.count ?? 0);

  return json({ user, pendingCount });
}
...
export default function FuelerLayout() {
  ...
  const navItems = [
    { to: "/fueler", label: "Dashboard" },
    { to: "/fueler/orders", label: `Orders \u00B7 ${pendingCount}` },
    { to: "/fueler/history", label: "History" },
    { to: "/fueler/profile", label: "Profile" },
  ];
    ...
    <div className="flex min-h-screen">
      ...
      <aside className={`${collapsed ? "w-16" : "w-56"} shrink-0 bg-slate-800 text-white transition-all duration-200 flex flex-col`}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700">
          {!collapsed && <h2 className="text-sm font-bold tracking-wider">FUELER</h2>}
