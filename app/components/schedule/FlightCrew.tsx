...
function pilotStatusDot(status: string | null): { bg: string; label: string } {
    switch (status) {
        case "confirmed":
            return { bg: "bg-green-500 dark:bg-green-900/30", label: "Confirmed" };
        case "assigned":
            return { bg: "bg-blue-500 dark:bg-blue-900/30", label: "Assigned" };
        case "checked_in":
            return { bg: "bg-emerald-500 dark:bg-emerald-900/30", label: "Checked In" };
        case "declined":
            return { bg: "bg-red-500 dark:bg-red-900/30", label: "Declined" };
        default:
    ...
    if (compact) {
        return (
            <span className="inline-flex items-center gap-1.5 text-[11px]">
                ...
                <span className={pilot_name ? "font-medium text-slate-700 dark:text-slate-200" : "text-slate-500 dark:text-slate-500"}>
