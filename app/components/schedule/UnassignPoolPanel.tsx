...
export function UnassignPoolPanel({ unassignedBookings, visibleCount, isNoFlyDay = false }: { unassignedBookings: UnassignedBookingRow[]; visibleCount: number; isNoFlyDay?: boolean }) {
  const [showAll, setShowAll] = useState(false);
  const visible = showAll ? unassignedBookings : unassignedBookings.slice(0, visibleCount);
  return (
    <div className="w-full max-w-md rounded-lg bg-white dark:bg-slate-800 p-6 shadow-xl dark:shadow-slate-900/50">
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="rounded-full bg-slate-100 dark:bg-slate-700 px-2 py-0.5 text-[10px] font-medium text-slate-500 dark:text-slate-400 dark:text-slate-500">{unassignedBookings.length}</span>
      </div>
      <p className="mb-3 text-[11px] text-slate-500 dark:text-slate-400 dark:text-slate-500">Drag bookings onto a flight card to assign them to a flight.</p>
      <div className="space-y-2">
        {visible.length === 0 && isNoFlyDay ? (
          ...
          <>
            ...
            {unassignedBookings.length > visibleCount && !showAll && (
              <button onClick={() => setShowAll(true)} className="w-full rounded py-1 text-[11px] font-medium text-blue-600 hover:bg-blue-50 dark:bg-blue-900/30 dark:hover:bg-blue-900/30 dark:bg-blue-900/30">
                Show all {unassignedBookings.length} bookings
              </button>
            )}
          </>
        )}
