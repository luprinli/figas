...
const statusStyles = {
  ...
  unknown: { dot: "bg-slate-400", text: "text-slate-500 dark:text-slate-400 dark:text-slate-500", label: "Unknown" },
};
          ...
          return (
            <div key={i} className="px-4 py-2.5 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                {item.detail && (
                  <span className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">{item.detail}</span>
                )}
