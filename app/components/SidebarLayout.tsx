...
export default function SidebarLayout({ title, userIdentity, navItems, footer }: Props) {
  return (
    <div className="flex min-h-screen">
      ...
      <aside className="w-64 bg-slate-800 text-white p-4 space-y-2" aria-label="Sidebar navigation">
        ...
        <nav className="space-y-1" aria-label="Main navigation">
          {navItems.map((item) => (
            <Link
              key={item.to}
              to={item.to}
              className="block px-3 py-2 rounded hover:bg-slate-700 transition-colors"
            >
              {item.label}
            </Link>
          ))}
        ...
        {footer && (
          <div className="border-t border-slate-600 mt-4 pt-4 px-3 space-y-2 text-sm">
            {footer}
          </div>
        )}
      </aside>
      <main id="main-content" className="flex-1 bg-slate-50 dark:bg-slate-700 dark:bg-slate-900">
        <Outlet />
      </main>
    </div>
  );
