...
## Flight-Loadsheet Consistency (Single Source of Truth)

The flight display and loadsheet read passenger data from the same canonical
SQL query (`findManifestsByFlightId`) and apply the same route-order filter
via shared utilities. This eliminates the passenger-count drift discovered
on 2026-07-20 where the loadsheet showed fewer passengers than the flight card.

### Shared Route Utilities

**File:** [`app/utils/scheduling/route-utils.ts`](../../app/utils/scheduling/route-utils.ts)

Two pure functions serve as the single source of truth for route matching:

| Function | Signature | Description |
|----------|-----------|-------------|
| `buildOrderedStopSequence` | `(flight, legs) => string[]` | Builds ordered, deduplicated stop list from flight metadata + leg codes. Prepends/appends `flight.origin_code`/`flight.destination_code` so passengers whose codes appear only at flight endpoints are not excluded. |
| `filterManifestsByRoute` | `(manifests, stopSequence) => T[]` | Filters manifest rows to only those where `origin_code` appears before `destination_code` in the stop sequence. Prevents "arriving before departing" on round-trip routes. |

### Consumers

| Consumer | File | Usage |
|----------|------|-------|
| Flight display | [`build-stop-activities.ts`](../../app/utils/scheduling/build-stop-activities.ts) | `buildOrderedStopSequence` ? `orderedCodes`; `filterManifestsByRoute` ? `routeValidManifests` |
| Loadsheet | [`create-loadsheet.server.ts`](../../app/utils/loadsheet/create-loadsheet.server.ts) | `buildOrderedStopSequence` ? `routeStops`; `filterManifestsByRoute` ? `routeMatchedRows` |

### Persistence Layer

The loadsheet stores filtered results in the `loadsheet_passengers` table as a
**persistent snapshot** (regulatory audit document). A reconciliation check at
[`ops.flight.$flightId.loadsheet.server.ts:25`](../../app/routes/ops.flight.$flightId.loadsheet.server.ts:25)
compares `countAssignedByFlightId()` against `loadsheet.total_pax` and
regenerates on mismatch � providing eventual consistency through lazy invalidation.

### Regression Tests

| Test | File | What It Verifies |
|------|------|-----------------|
| Integration | [`flight-loadsheet-consistency.test.ts`](../../tests/integration/scheduling/flight-loadsheet-consistency.test.ts) | 3-passenger STY?PHD?ALB?DGS?CHR?STY route: `findManifestsByFlightId` and `createLoadsheetFromFlight` return identical counts and weights |
| E2E | [`flight-loadsheet-consistency.spec.ts`](../../tests/e2e/flight-loadsheet-consistency.spec.ts) | Static check: every flight card's passenger count matches its loadsheet |

### Invariant

- **CI-8:** `buildOrderedStopSequence` and `filterManifestsByRoute` are the ONLY functions that may build a stop sequence or filter manifests by route. No other file may duplicate this logic. (Enforced by code review / architectural guardrail AG-5.)
...
| [`app/utils/scheduling/route-utils.ts`](../../app/utils/scheduling/route-utils.ts) | `flight-loadsheet-consistency.test.ts`, `auto-build.test.ts` |
| [`app/utils/loadsheet/create-loadsheet.server.ts`](../../app/utils/loadsheet/create-loadsheet.server.ts) | `flight-loadsheet-consistency.test.ts` |
